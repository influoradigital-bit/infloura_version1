...
**Status:** ready to build. **Baseline commit:** `8c7b18b` (Phase A, Priya 10/10). **Date:** 2026-09-04.
...
This document is the contract for three engineers working in parallel (backend `influora-api`, AI service `influora-ai`, frontend `src/`). It names every file to add or change, every step, and every test. Nothing in it requires a Swapnil decision except the two flagged in section 12.
...
No change to the cap logic. `send_routine_reply` and `draft_reply` cost nothing extra; the brief extraction route is billed through the existing daily gate keyed by `workspace_id` (creator user id).
...
1. **Brand-facing "sent by Meera" marker.** Default: stripped for brands (2.8). Flip by removing the strip.
2. **Referral incentive on creator-brought brands.** The secure-link redemption records `introduced_by_creator_profile_id` on the campaign? Not in this spec; the plan recommends a fee cut and a first-invite window. Add a `introduced_by` column on `campaigns` only after the ruling; nothing in Phase B depends on it.
