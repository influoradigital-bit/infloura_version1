...
public class PlatformFeeService {
  ...
  public FeeDeductionResult deductAtRelease(
    ...
    if (split.platformFee().signum() > 0) {
      ...
      // D14 Doc#3b — creator-leg commission invoice, created AFTER the PLATFORM_FEE posting above
      // has actually succeeded, inside this same @Transactional method as the posting itself.
      // Idempotency key "release-fee:" + escrowHoldId already dedupes the posting; the invoice
      // service does its own additional findByEscrowHoldIdAndLeg check before minting a statutory
      // number, so a retried release can never double-issue.
      commissionInvoiceService.createCreatorLegAtRelease(
          escrowHoldId, creatorUserId, feeBps, split.platformFee(), feePosting);
    }
