 ...
 *
 * <p><b>Brand-safety columns:</b> {@code brandSafetyScore}/{@code garmFlags}/{@code
 * contentSentiment} are nullable in the V22 migration and are populated by {@code
 * ScoreCalculationJob} ONLY when the OFF-by-default brand-safety feature flag is enabled (see
 * {@code BrandSafetyScoringProperties} / the job's class javadoc). When the flag is off — or when a
 * given creator wasn't among the per-run brand-safety cap, or influora-ai was unreachable — these 3
 * columns stay {@code null} on the row. A {@code null} always means "not scored", never "scored
 * safe".
 *
...
public class CreatorScore {
    ...
    // Brand Safety Score — populated only when the brand-safety feature flag is on (see class
    // javadoc). Nullable, no NOT NULL constraint per V22 migration; null means "not scored".
    ...
    public static final class Builder {
        ...
        /** Populated by ScoreCalculationJob only when the brand-safety flag is on — see class javadoc. */
