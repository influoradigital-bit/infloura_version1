...
public class EmailOutbox {
    ...
    /**
     * D5: leases this row until {@code leaseUntil} without touching {@code status}/{@code
     * retryCount} — {@code EmailWorker}'s claim step calls this (inside a short, DB-only
     * transaction) right after selecting a batch via {@code FOR UPDATE SKIP LOCKED}, so a
     * concurrent/subsequent poll's {@code WHERE status = PENDING AND (nextRetryAt IS NULL OR
     * nextRetryAt <= now)} clause skips these rows until either the claim completes ({@link
     * #markSent()}/{@link #markFailed(String)}, both of which overwrite {@code nextRetryAt}) or
     * the lease itself expires (crash-recovery: if the worker dies mid-batch, the row becomes
     * visible again once {@code leaseUntil} passes, same as any other retry).
     */
    public void markClaimed(Instant leaseUntil) {
