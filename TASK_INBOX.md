...
## 🆕 SWAPNIL DIRECTIVE — T-RATECARD-0903 (creator rate card: bugs + v2) — 2026-09-03

**Origin:** Swapnil, direct, after a product review of "should creators post 'open to job at ₹X'". Ruling from two independent experts + Tejas: **no creator job board, no second money path, no public price lists.** Instead: rate card v2 + availability + budget-fit, and a later "Hire directly" that creates a private single-creator campaign so contracts / Secure Payments / invoices stay unchanged. Priya's code trace: `rateMin`/`rateMax` are one pair per creator (V6 `creator_profiles.rate_min/rate_max`), written by onboarding and profile-edit; **no availability concept exists anywhere.**

### Ticket A — bugs in shipped code (P1, vikram, ledger F-0497 + F-0498)

| Pri | ID | Where | Symptom | Status |
|---|---|---|---|---|
| P1 | F-0497 | `PortfolioService.java:691` | Anonymous `GET /portfolio/{username}` returned rate numbers even when visibility was `brands_only` (the default); FE only drew a "sign in" card over them | **FIXED 2026-09-03** — `buildRateCard` now takes `publicView`; 3 regression tests in `PortfolioServiceTest`, falsified against the unfixed code, 20/20 green. Uncommitted on this branch. |
| P1 | F-0498 | `PortfolioService.java:782` | Portfolio PATCH accepts a per-row `rateCard` list but persists only row 0's min/max; label + currency + other rows silently dropped; `buildRateCard` then fabricates two identical Post/Reel rows. Portfolio path also skips the `min > max` INVALID_RATE_RANGE check onboarding and profile-edit enforce; no row cap. | OPEN — stop collapsing the list (see Ticket B storage), add the range check + a row cap on the portfolio path. |

Kavya: review both; the F-0497 test must be kept (it is the only server-side assertion of the visibility contract).

### Ticket B — Rate card v2 + availability (P2, vikram + ananya, 30-day scope)

1. **Per-deliverable rates** (Reel / Story / Post / bundle, each min+max+currency) and an **`openToOffers` toggle**, both stored inside the existing `creator_profiles.portfolio_settings_json` blob (`CreatorProfile.java:58`, `PortfolioService.loadSettings` ~761). **No new `@Column`s** on `CreatorProfile` — that is the unnamed-camelCase-JSON-column boot crash from before. Keep `rate_min`/`rate_max` as the derived floor/ceiling so discovery sort and `rateOverlap` keep working.
2. `openToOffers` and any new visibility field ship as **nullable `Boolean` with an explicit default**, not a primitive — `PortfolioSettings` is parsed with a bare `ObjectMapper` and a primitive would silently mark every existing creator "not open".
3. **Availability badge** (`Available this month` / `Booked`) on the creator profile, discovery card, and brand creator-profile page (`brand-creator-profile.tsx:391` already has a TODO for per-type rates and ships `rates: {}` empty).
4. **Budget-fit filter** in brand discovery on top of the existing rate sort (`CreatorDiscoveryService.java:1049`) and score nudge (~746). Creators with no rate card are currently kept by `rateOverlap`'s `OR isNull`; decide explicitly whether the new filter keeps or drops them and nudge empty rate cards in the creator UI.
5. **Exact prices stay hidden** from brands that have not created a campaign; show the existing "Fits within your budget" / "May exceed budget" strings instead.
6. **Portfolio editor** currently has no rate-entry UI (`creator-portfolio-editor.tsx` only exposes the visibility select); v2 entry lives there or in profile-edit, not both.
7. Rates exist only in `src/lib/api.ts` (`:4042` etc.); `meera-api.ts` has zero rate references — decide deliberately whether Meera reads the new list (read-only is fine; Meera must never set budgets on either side).

**Instrument before deciding anything further (dev):** % of campaigns with 0 applications after 7 days, median applications per campaign, invite-accept rate, time to first application. Creator-posted packages are only reconsidered if >60% of campaigns get 3+ applications in week one.

**Not in scope, by ruling:** creator job board, campaign-less collaborations, public price lists, Meta-imported creators with auto-published rates.

---

## 🆕 SWAPNIL DIRECTIVE — T-CREATORCONNECT-0902 (creator marketplace connect flow) — 2026-09-02

**Task:** `.proof-os/tasks/T-CREATORCONNECT-0902/TASKS.md` · **Origin:** Swapnil, direct · **Goal:** "make this flow ready to use"

Discover shows Meta-sourced Instagram creators with an **Unverified with Influora** badge + **Connect this creator** CTA → enquiry to admin → admin invites creator → creator joins → brand emailed with a create-campaign link. Marketplace API client ships dark (`META_CREATOR_MARKETPLACE_ENABLED=false`) until App Review unblocks; flow runs on Business Discovery + admin imports today.

| Owner | Scope | Status |
|---|---|---|
| vikram | migration, `/creators/external/*`, `/admin/creator-connections/*`, JOINED hook, 3 email templates, flagged `CreatorMarketplaceClient` | DONE — Kavya PASS |
| ananya | Discover Instagram tab + badge + CTA dialog, `?creatorId=` on new campaign, admin Creator connections page, api.ts | DONE — Kavya PASS (P2 fixed) |
...
## 🔧 PHONE-0829 — phone capture (IN PROGRESS, dispatched 2026-08-29)

**Origin:** Swapnil found admin cannot see creator mobile numbers. Root cause traced: **no code path
has ever written `users.phone_number`** — the column exists since `V2__core_auth.sql` as a
placeholder for the never-built phone-OTP login (same dead feature as the MSG91 SMS credentials that
feed no code — no SMS client class exists).

Three phone fields, three different states:
| Field | State |
|---|---|
| `users.phone_number` (VARCHAR(20) UNIQUE) | ☠️ never captured — zero setter callers, NULL for every user |
| `workspaces.phone` (V20260718180000) | ✅ works — `WorkspaceService.java:122` → `Workspace.updatePhone()` |
| `shipments.phone` | ✅ works — captured in both shipping forms |

**Data-loss bug found:** brand onboarding RENDERS a "Mobile number" field
(`onboarding-steps.tsx:706`), marks it **Required** (`:464`), validates `/^[6-9]\d{9}$/` (`:465`) —
and `brand-onboarding.tsx` never sends it; the onboarding DTO has no phone field. Every brand that
ever onboarded had their typed mobile silently discarded.

**Scope:** P1 creator phone capture (settings → `users.phone_number`) · P2 brand onboarding persist
(→ `workspaces.phone`, reusing the existing write path) · P3 admin DTO + panel display.
Owners: Vikram (`influora-api/**`), Ananya (`src/**`).

**CTO ruling — phone is stored PLAINTEXT**, exactly as `User.email` is stored today.
`EmailPhonePiiCipher` is a live-but-unwired SCAFFOLD (bound to no entity). Encrypting phone alone
would need a widened column + blind-index unique index and would leave equally-sensitive email
...
**Symptom reported:** admin login shows "wrong credentials" for `influoradigital@gmail.com`.
**NOT a wrong password** — `failed_login_attempts = 0` proves the password check passes (a wrong
password increments it). Actual cause: the row is `SUPER_ADMIN` with `mfa_enabled = 0`, and
`ADMIN_MFA_ENFORCE_ON_LOGIN` defaults **true**, so login is rejected with `MFA_ENROLLMENT_REQUIRED`
— which the admin login page renders as a generic credentials error.

`AdminSecurityProperties.java:22` **predicted this exact deadlock in its own javadoc** ("If any
existing SUPER_ADMIN/ADMIN row has mfa_enabled = false today, this change locks that account out of
login entirely with no in-app recovery path… Before deploying with this flag left at its default
true, confirm no such row exists"). That pre-deploy check never happened.

**Workaround given to Swapnil:** temporarily flip role to SUPPORT (exempt from the gate) → log in →
enrol MFA → flip back to SUPER_ADMIN.

**Permanent fixes still owed (NOT yet built):**
1. An ops provisioning script for the first admin — `AdminUser.java:79-83` promises one ("provisioned
   out-of-band (ops script / future AdminUserController)"); it was never written.
2. A "reset another admin's MFA" endpoint — `AdminAuthService`'s own javadoc lists it as a known gap.
3. Surface `MFA_ENROLLMENT_REQUIRED` distinctly in the admin login UI instead of the generic
   credentials message (`admin-login.tsx:67` falls back because the error envelope is inconsistent).
