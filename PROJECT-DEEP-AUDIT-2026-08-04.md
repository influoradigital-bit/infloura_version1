...
> **Admin remediation update #4 — 2026-08-04 (proof-os task `admin-leftovers-0804`, priya sign-off).** The two remaining pure-FE-wiring items (already-real backend, no UI) are now wired (⬜→✅ ×2):
> - **Revenue by range** — `financeApi.getRevenue(start,end,groupBy)` (`GET /admin/finance/revenue`, `AdminRevenueController`) → new `RevenueRangePanel` ("Revenue Range" tab): date-range + groupBy inputs, on-demand fetch, `RevenueSnapshot[]` table. Distinct from the trailing-12 `getFinancialSummary` KPI.
> - **Entity audit lookup** — `auditApi.getByEntity(type,id)` (`GET /admin/audit/entity/*`, `AuditLogController`) → new `EntityAuditPanel` ("Entity Audit" tab): entityType/entityId inputs → `AuditLogEntry[]` table.
> Both are interactive (fetch-on-submit), so intentionally NOT in `useFinanceConsole`'s mount-time fan-out. Admin ⬜ 7 → **5**; ✅ 24 → **26**. Verified: `npx tsc --noEmit` exit 0 + reachability greps (both wrappers now have a real caller; tabs wired; 0 mock arrays). Ceiling: BELIEVED (static; no live HTTP).

> **Admin remediation update #5 — 2026-08-04 (proof-os task `admin-missing-backend-0804`, CFO rohan feasibility + CTO priya sign-off + 3-round red-team by kabir).** Of the 5 "blocked" backend items, a CFO primary-source assessment found **2 genuinely buildable** — both now built:
> - ✅ **Reconciliation → Working.** New read-only `GET /admin/finance/reconciliation?date=` (`AdminFinanceService`) diffs internal `Payout`/`WalletTopUp` ledger rows against the persisted Razorpay gateway record (`razorpayPayoutId`/`razorpayOrderId`), emitting MATCHED/MISMATCH/PENDING. Wired to a new "Reconciliation" console tab (`ReconciliationPanel`). Fails safe on gateway-fetch error. **kabir cleared it money-safe.**
> - 🟡 **Payout-retry → Partial.** New `POST /admin/finance/payouts/{id}/retry` (SUPER_ADMIN-only) + a real money-safety **bug fix** kabir surfaced (REJECTED/CANCELLED payouts weren't re-credited → creator lost money). Idempotent (fresh per-attempt keys; retry-guard reservation before the gateway call). **Three red-team rounds fixed 4 findings** (F-0080 lost-money re-credit collision, F-0081 crash-strand, F-0082 ledger-key VARCHAR(64) overflow, F3 RBAC). 31/31 unit tests + a key-length guard. **Classified PARTIAL, not Working:** it moves real money and its DB-level persistence + sweep-vs-retry concurrency can only be *believed* here — two verifications need a live MySQL/RazorpayX environment (kabir's residuals) before it touches real payouts. Reachable via a "Retry payout" action on MISMATCH reconciliation rows.
> - 🔴 **Still MISSING — 3, honestly blocked (NOT fabricated):** **TDS/26Q** — no TDS is ever deducted or stored on any payout/invoice (`PayoutService.resolveNetPayoutAmount` is gross-minus-commission only); a 26Q report would be *fabricated compliance output* — needs a deduction step first. **Acquisition** — no ad-spend, no signup-source attribution, no `Referral` table (reconfirmed). **Escrow-mutate** — the mutation code *exists* and is live via the audited dispute-mediated model (`EscrowService.adminReleaseForDispute` etc.); the raw admin bypass is a deliberate, security-reviewed *open design decision* pending Swapnil/Priya, not backend work.
> Verified by my own oracles: `mvn -o -q clean compile` exit 0, `PayoutReconciliationServiceTest`+`AdminFinanceServiceTest` 31/31, `npx tsc --noEmit` exit 0, FE reachability greps. New standing gate `.proof-os/gates/payout-money-safety.sh`. Ceiling: reconciliation BELIEVED-money-safe; payout-retry BELIEVED-pending-live-DB.
...
*Admin row is post-remediation (2026-08-04): `admin-fix-0804` 🔴 2 → 0 (phantom endpoints deleted, total 37 → 35); `admin-console-0804` ✅ 17 → 24 / ⬜ 15 → 8 (Finance/Escrow/Revenue console built, **7** real routes wired — `getRevenue` stays orphan); `admin-backend-0804` ⬜ 8 → 7 / 🟡 3 → 4 (real data-backed Growth endpoint — `GET /admin/marketing/growth`); `admin-leftovers-0804` ✅ 24 → 26 / ⬜ 7 → 5 (`getRevenue` + `audit-entity` wired); then `admin-missing-backend-0804` ✅ 26 → 27 / 🟡 4 → 5 / ⬜ 5 → 3 (reconciliation built + money-safe → Working; payout-retry built + 3-round red-team-hardened but live-DB-unverified → Partial). The remaining 3 MISSING are honestly blocked — TDS + acquisition on missing data, escrow-mutate on a security decision (see the remediation updates at the top).*

**Completion metrics (transparent, code-truth):**
- Fully working: **161 / 194 = 83.0%**
- Wired to a real backend (working + partial): **178 / 194 = 91.8%**
- Weighted (partial = ½): **169.5 / 194 = 87.4%**

### The one-line story
**The API wiring is excellent — the problems are not connection bugs.** All 140 typed `request()` calls in the main frontend client resolve to a real backend method+path (**zero phantom endpoints**). *(As of the 2026-08-04 remediation the admin finance/escrow/revenue console is now built and reachable — see updates #2–#5.)* The remaining gaps are: money flows that are code-complete but blocked on **infra/config** (live Razorpay keys) or **deliberate security scope exclusions**, analytics/scoring placeholders, a few genuinely blocked backend features (TDS deduction, acquisition data infra, the escrow-mutate security decision), and a handful of unbuilt user flows.
...
## 4. What's PARTIAL (wired, but stub / placeholder / gated) — 18

### Admin (5)
- 🟡 **Payout-retry** *(BUILT 2026-08-04, `admin-missing-backend-0804` — was ⬜ Missing)* — `POST /admin/finance/payouts/{id}/retry` (SUPER_ADMIN-only) re-initiates a definitively-failed creator payout via the live RazorpayX rail, reusing the existing `reconcileOrphanedPendingPayout` retry path with a fresh per-attempt idempotency key. Includes a real money-safety fix (REJECTED/CANCELLED payouts now re-credit the wallet). **PARTIAL, not Working:** it moves real money and cleared 3 red-team rounds (F-0080/81/82) at code level with 31/31 unit tests + a key-length guard, but two money-safety verifications need a live MySQL/RazorpayX environment (a real-DB retry-persistence + crash-resume test; the sweep-vs-in-flight-retry concurrency race) — believed, not proved. Reachable via the Retry action on MISMATCH rows in the Reconciliation tab. `PayoutReconciliationService.retryFailedPayout`.
...
## 5. What's MISSING — 16 *(was 36; −2 creator `creator-fix-0804`, −4 creator `creator-deliv-0804`, −2 creator `creator-missing-0804`, −7 admin `admin-console-0804`, −1 admin `admin-backend-0804` [growth], −2 admin `admin-leftovers-0804` [getRevenue + audit-entity], −2 admin `admin-missing-backend-0804` [reconciliation → Working, payout-retry → Partial])*

**Important distinction:** most "Missing" items are **orphan backend routes** (backend built, no UI wired) — often intentional (email-link handlers, admin/Meera-only paths, alternate endpoints). Only a few are genuinely unbuilt user-facing features. The genuinely-missing UI is what matters. *Admin's MISSING is now **3** (was 15): across `admin-console-0804` / `admin-backend-0804` / `admin-leftovers-0804` / `admin-missing-backend-0804`, the console + growth + getRevenue/audit-entity + reconciliation + payout-retry were all built. The remaining **3** are honestly blocked — verified from primary source by CFO rohan — and building them now would fabricate financial/compliance output, which is worse than an honest "not built":*
- ⬜ **TDS / 26Q** — no TDS is ever deducted or stored on any payout/invoice (`PayoutService.resolveNetPayoutAmount` pays gross-minus-commission only; `pan`/`gstin` captured but never used for withholding). A 26Q return over this data is fabricated compliance. **Needs a tax-deduction step wired into the payout path first, then the report.**
- ⬜ **Acquisition** (CAC / source attribution) — no ad-spend ledger, no signup-source/UTM/referrer column on `User`, no `Referral` table (reconfirmed). **Needs a data-collection project**, not a controller.
- ⬜ **Escrow-mutate** (direct admin hold/release/refund) — the money-moving code *exists and is live* via the audited dispute-mediated model (`EscrowService.adminReleaseForDispute`/`adminRefundForDispute`/`adminSplitForDispute` ← `AdminDisputeController`). The raw non-dispute admin bypass is a **deliberate, security-reviewed open design decision** (Kabir's phase-1 escrow review) pending Swapnil/Priya — a product/security call, not backend work.

### Genuinely unbuilt user-facing features (build these)
- ✅ **Admin Finance / Escrow / Revenue console UI** *(BUILT 2026-08-04, `admin-console-0804` — was ⬜)* — new `RevenuePage` / `FinanceConsole` / `useFinanceConsole` at `/admin/revenue` (routed `admin-console.tsx:58`, nav `AdminLayout.tsx:64`) consumes **7** real GET routes: `/admin/dashboard/financial` (also feeds the revenue-trend table), `/admin/finance/escrow`, `/admin/escrow/flagged`, `/admin/campaigns/at-risk`, `/admin/campaigns/hype/ops`, `/admin/moderation/suspensions`, `/admin/marketing/reputation`. Live data only, no mock arrays; `tsc` green. *(Update `admin-leftovers-0804`: the two previously-orphan routes are now wired too — `/admin/finance/revenue` via the "Revenue Range" tab (`RevenueRangePanel`, on-demand custom date range) and `/admin/audit/entity/*` via the "Entity Audit" tab (`EntityAuditPanel`, on-demand lookup). The console now consumes 9 real routes; 0 orphan.)*
...
- ✅ **Reconciliation** + 🟡 **Payout-retry** *(BUILT 2026-08-04, `admin-missing-backend-0804`)* — reconciliation is a real read-only ledger-vs-gateway diff (`GET /admin/finance/reconciliation`, Working, money-safe); payout-retry is built + 3-round red-team-hardened (Partial, pending live-DB verification). Both reachable in the console Reconciliation tab. See the §4 / update-#5 detail.
- ⬜ **Admin marketing Acquisition** + **TDS/26Q** + **escrow-mutate** *(blocked — see the 3 bullets above §5's "Genuinely unbuilt" header)* — acquisition needs signup-source/ad-spend/referral data infra; TDS needs a tax-deduction step in the payout path (no TDS is tracked today); escrow-mutate is a deliberate security decision (dispute-mediated model is the audited path). Appeal review also needs the appeal domain model.
...
2. ~~**Build the Admin Finance/Escrow/Revenue console UI**~~ — ✅ **DONE 2026-08-04**: `/admin/revenue` console (`admin-console-0804`, 7 routes) + Growth endpoint (`admin-backend-0804`) + `getRevenue`/`audit-entity` tabs (`admin-leftovers-0804`) — **9 real routes wired, 0 orphan**; `tsc` green. Then `admin-missing-backend-0804` built **reconciliation** (Working, money-safe) + **payout-retry** (Partial, live-DB-verification pending). Remaining admin work is now genuinely non-code: **TDS** needs a tax-deduction step wired into the payout path before a 26Q report can exist; **acquisition** needs a signup-source/ad-spend/referral data-collection project; **escrow-mutate** + the 2 `501` gates need a security/product decision; the `kpi_daily_snapshot` dashboard deltas need a DB migration + snapshot job. Plus: run the live-MySQL/RazorpayX integration test that closes out payout-retry.
