...
```
dev → proof-os | T-F0329-GATES: F-0240/F-0241/F-0243 gates were all three FALSE-GREENS on CLOSED records (observed exit 0 under injection); repaired with rendered-behaviour vitest legs + frozen known-bad self-falsification; each now exits 1 on injection, 0 on the real tree | .proof-os/gates/F-024{0,1,3}-*.sh, .proof-os/gates/F-024{0,1,3}.*.spec.tsx, .proof-os/gates/_f0240_chain.py, logs in .proof-os/tasks/T-F0329-GATES/ | DONE | NEXT: the `won.t`-style ERE-dot-as-apostrophe pattern is a repo-wide gate hazard — sweep the other promoted gates for it
```

```
dev → proof-os | F-0334 closed: .proof-os gate fixtures were being collected by the product suite and by build.sh's `npm test` leg. Fixed with vitest.config.ts exclude '**/.proof-os/**' (1 line + comment) plus .proof-os/gates/vitest.gates.config.ts, which DERIVES from the project config and drops only that entry — gates pass --config to it. Collection 118 -> 115 files, zero .proof-os paths; full vitest run 115 files / 698 tests pass; build.sh exit 0 (tsc/build/tests all proved). F-0237 and F-0238 also materialise specs under .proof-os/gates/ and would have gone unavailable — both repointed at the gates config and re-verified at exit 0 | vitest.config.ts, .proof-os/gates/vitest.gates.config.ts, .proof-os/gates/F-023{7,8}-*.sh | DONE | NEXT: any FUTURE gate that materialises a spec under .proof-os/gates/ must pass --config .proof-os/gates/vitest.gates.config.ts or it will silently report "No test files found"
```
- [dev · T-F0335] F-0050 gate was name-locked (KanbanView rename walked past it at exit 0, observed). Repaired to a class-detector: .proof-os/gates/_nested_component.js parses the TSX with the project TypeScript and reports any function/class LITERAL declared in a render body and mounted as a JSX tag, no identifiers enumerated. 5 known-bad + 6 known-good fixtures frozen at .proof-os/gates/fixtures/F-0335/ and run through the detector on every invocation (self-falsifies both ways). Observed exits: new-name injection 1, original three-identifier 1, real tree 0, known-good probes 0. Subject file sha256 restored and verified. Log: .proof-os/tasks/T-F0335/F-0335.inject.log
