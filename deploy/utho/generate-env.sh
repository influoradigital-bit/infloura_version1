...
# T-GOLIVE-0918 [meera - 2026-09-18] -- REPAIR ROUND 3 HIGH: this used to write
# INFLUORA_BRANDSAFETYSERVICETOKEN_SIGNINGSECRET, Spring's relaxed-binding alias, which has no
# placeholder anywhere in application.yml. Both compose files forward BRAND_SAFETY_SERVICE_TOKEN_SECRET
# instead (the name application.yml:279 actually binds) and no longer forward the old name at all,
# so a fresh env generated by this script under the old name left BRAND_SAFETY_SERVICE_TOKEN_SECRET
# unset -- influora-api fails SecretsStartupValidator and does not boot outside dev. Renamed here
# so generate-env.sh, both compose files and application.yml:279 all agree on one name end to end.
# Note: this is a heredoc WITHOUT a quoted delimiter (line 31), so a literal dollar-brace token in
# this comment would itself be shell-expanded into the generated .env file -- avoided here on
# purpose, see docker-compose.utho.yml:165 / docker-compose.utho-shared.yml:191 for the actual
# placeholder syntax. Source: BrandSafetyServiceTokenProperties.java:42-56.
...
# (application.yml's own dollar-brace-VAR-colon-default placeholder), not a generic REPLACE_ME -- otherwise an
