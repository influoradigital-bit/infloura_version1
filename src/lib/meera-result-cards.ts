 ...
 *     looks like, Script, Caption, Before you shoot, Why this works, and the Shot/Say/On screen
 *     markers inside a beat) stay in English per the persona file, so only the labels are matched
 *     case/whitespace-insensitively; the surrounding prose is read as opaque text either way
 ...
 *     `<start>-<end>s. Shot: … . Say: "…". On screen: ….` shape, or one with an empty Shot/Say/
 *     On-screen part
...
/**
 * `0-3s. Shot: <camera angle> - <action>. Say: "<exact line>". On screen: <text>.` — the exact
 * layout `creator_persona.py`'s "Full script format" prescribes for a beat line. Non-greedy up to
 * the first `.` after `Shot:` (the persona's own shot text has no reason to contain a period);
 * the Say text is read up to the first closing quote immediately followed by `.`; the on-screen
 * text is whatever is left, with one optional trailing `.` stripped (the format's own closing
 * punctuation, not part of the overlay text).
 */
const BEAT_RE = new RegExp(
  `^(\\d+)-(\\d+)s\\.\\s*Shot:\\s*(.+?)\\.\\s*Say:\\s*${OPEN_QUOTE}(.+?)${CLOSE_QUOTE}\\.\\s*On screen:\\s*(.+?)\\.?\\s*$`,
  'i',
);
...
export function parseMeeraScript(text: string): ParsedMeeraScript | undefined {
  ...
  while (i < lines.length) {
    ...
    const onScreen = match[5].trim();
    if (!shot || !say || !onScreen) return undefined;
    beats.push({ from, to, shot, say, onScreen });
    i++;
  }
