...
export interface UseFlagQueueResult {
  ...
  totalCount: number;
  /**
   * Unfiltered count of unresolved flags (PENDING + ESCALATED), for a
   * queue-depth badge. ESCALATED is unresolved work — it counts everywhere
   * PENDING counts as outstanding (Priya ruling, 2026-07-15; mirrors the
   * backend's `countByStatusIn({PENDING, ESCALATED})` reviewBacklog).
   */
...
export function useFlagQueue(): UseFlagQueueResult {
  ...
  return {
    ...
    unresolvedCount: allFlags.filter(
      (f) => f.status === ContentFlagStatus.PENDING || f.status === ContentFlagStatus.ESCALATED,
    ).length,
    isLoading,
    error,
    filters,
