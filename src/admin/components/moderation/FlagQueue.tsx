...
type PillTone = 'success' | 'warning' | 'destructive' | 'neutral' | 'escalated';
...
const PILL_TONE_CLASSES: Record<PillTone, string> = {
  ...
  // ESCALATED: unresolved work reassigned to a senior admin — must read as
  // urgent/open, never as resolved (success) or same-as-PENDING (destructive).
  // `accent-foreground` is the existing brand indigo token (already used for
  // solid-fill treatments elsewhere, e.g. button.tsx/sidebar.tsx) — #5b4db3
  // under white text is ~6.6:1 contrast, clears WCAG AA (4.5:1) with margin.
  escalated: 'bg-accent-foreground',
};
...
function statusPillTone(status: ContentFlagStatus): PillTone {
  switch (status) {
    case ContentFlagStatus.PENDING:
      return 'destructive';
    case ContentFlagStatus.ESCALATED:
      return 'escalated';
...
export default function FlagQueue({ className }: FlagQueueProps) {
  const { flags, totalCount, unresolvedCount, isLoading, error, filters, setFilters, sort, setSort } = useFlagQueue();
        ...
        <div className="flex flex-1 flex-col gap-2 sm:flex-row sm:items-center">
          ...
          >
            ...
            <SelectContent>
              ...
              <SelectItem value={ContentFlagStatus.ESCALATED}>Escalated</SelectItem>
