...
export default function KpiCard({
  ...
  // `unknown` (no snapshot baseline yet — the value came in as `null`) is a
  // distinct state from `neutral` (a genuine, measured 0% flat reading). It
  // must never borrow the up/down trend icon or the positive/negative
  // colors — that would read as "measured, and flat/good", not "unmeasured".
  const isUnknownChange = changeType === 'unknown';
  const TrendIcon = changeType === 'positive' ? TrendingUp : changeType === 'negative' ? TrendingDown : changeType === 'neutral' ? Minus : HelpCircle;
  ...
  const deltaClasses = cn(
    ...
    changeType === 'positive' && 'bg-success-foreground text-white',
    changeType === 'negative' && 'bg-destructive-foreground text-white',
    changeType === 'neutral' && 'bg-foreground text-white',
    // Muted, outlined pill for "unknown" — visually distinct from every
    // colored/solid trend state so it can never be mistaken for a measured value.
    isUnknownChange && 'border border-border bg-muted text-muted-foreground',
  );
  ...
  return (
    ...
    >
      <Card className={cn('gap-3 p-5', className)}>
        ...
        {/* Explicit `=== null` check, never a truthiness/relational one: `null >= 0` is
            `true` in JS, and `null || x` would swallow a genuine `0`. Rendered as an
            honest "—" with no trend icon and no positive/negative color — distinct from
            both a real 0% (above) and from "no change tracked at all" (change undefined). */}
        {change === null && (
          <span className={deltaClasses} aria-label="No comparison data available">
