...
3. **`AffiliateEarningsService.java:79-85` and `:123-126`** reference *"`RedemptionService`'s identical `self` field"* and a shared *"[SEC: Kabir, Wave D task D4 HIGH-1 — FIXED]"* self-invocation transactional fix — describing wiring in `RedemptionService` that **does not exist**. <!-- doc-claim-ok: the class exists; what is denied is the `self` field and the self-invocation transactional fix INSIDE it, which is still accurate. This ruling documents other javadocs being wrong, so the negative is the point. -->
