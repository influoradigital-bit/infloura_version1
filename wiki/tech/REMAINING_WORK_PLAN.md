...
| B4 | **Vikram** | `AudienceDemographics` entity + migration (V26+) + `AudienceDemographicsJob` (weekly) + `GET /analytics/creators/{id}/demographics`, which now ships at `AnalyticsController.java:84` [CORRECTED 2026-09-06, doc-stale-doc-claim: this row listed the endpoint as outstanding work and described a response the controller does not return]. | B1 | Migration live-verified by Meera; endpoint workspace-authed; dashboard demographics panel can light up. |
