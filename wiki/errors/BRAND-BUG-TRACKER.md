...
| PR-1 | 🔴 HIGH | Creator profile (brand view) | Every creator renders 6 fabricated zeros under "Based on verified brand collaborations" | Ananya | **DONE** | `BrandF.md` §87 — fixed; Priya REJECTED twice (endpoint 404'd on real ids, then missed a userId-shaped caller) before approving on the 3rd pass (1 follow-up logged: F-0157) |
...
| BL-2 | 🔴 HIGH · static-certain | Billing | Cancellation never terminates — renewal job re-allots Pro credits forever | Vikram | **DONE** | `BrandF.md` §105 — fixed; Priya REJECTED once (re-subscribe latch could wrongly cancel a paying customer) before approving via mutation-testing (1 follow-up logged: F-0159) |
| BL-3 | 🔴 HIGH · static-certain | Billing | No idempotency/lock on checkout — double-submit orphans a live Razorpay subscription | Vikram | **DONE** | `BrandF.md` §105 — fixed after 5 review rounds; uncovered and fixed a foundational IdempotencyService bug shared by 25+ money-path callers (F-0160–F-0162) |
