...
>
> ⚠️ **What is NOT pinned: the call site.** Reverting `creator-settings.tsx` back to `removeItem('creator_token')` would leave all 267 tests passing. Pinning it needs a render harness for that page, which does not exist — the same situation CR-29 was opened for on `creator-chat.tsx`, and it should be handled the same way (its own ticket) rather than left implied. **Do not read "tests pass" as evidence this call site is protected.**
