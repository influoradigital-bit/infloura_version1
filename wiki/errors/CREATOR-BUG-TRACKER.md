...
**By status:** 4 `IN QA` · 36 `IN VERIFY` · 2 `IN PROGRESS` · 1 `ASSIGNED` · 3 `BLOCKED` · **0 `OPEN`** · **0 `DONE`**
...
>
> **The box now serves `index-Cr-qruCI.js`, NOT Wave 2's `index-B_x5CUtn.js`.** `grep priya_sharma` in the new bundle → **0** (CR-06 gone from the deployed build); `POST /api/v1/client-errors` → **202** (CR-11 endpoint + Waves 3–6 live); API health **200** after boot, so the **CR-35 Flyway escrow-backfill migration ran clean against the box's real MySQL** (a failed migration blocks boot). Dev profile → the CR-39 cleanup job stays **dry-run**.
>
...
> 1. **Waves 1–2 ARE deployed.** `http://200.141.1.6` serves `index-B_x5CUtn.js` at **2,697,823 bytes**, which matches the recorded Wave 2 build (2,697.80 kB). The claim elsewhere in this file that the box still serves the pre-Wave-1 `index-NdzlUg4U.js` (~2.68 MB) is **wrong** — it has been superseded. **⚠️ 2026-07-29: superseded again — the box now serves `index-Cr-qruCI.js`, not `index-B_x5CUtn.js`; see the deploy banner at the top of §9.**
