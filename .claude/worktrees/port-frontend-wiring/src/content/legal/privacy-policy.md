# Privacy Policy

> ⚠️ **v0 DRAFT — PENDING INDIAN LEGAL COUNSEL / CA REVIEW. NOT LEGALLY BINDING. Ships `noindex` until validated.**

This policy explains what personal data we collect, why, and how you can control it under India's Digital Personal Data Protection Act, 2023.

**Last updated: 2026-07-13**

---

## 1. Who we are

**Influora Digital Private Limited** ("Influora," "we," "us") is the **Data Fiduciary** for the personal data we collect through our platform, as defined under the Digital Personal Data Protection Act, 2023 ("DPDP Act"). Registered office: `[REGISTERED ADDRESS — TBD]`. CIN: `U73100MH2024PTC434321`.

## 2. What data we collect

| Category | Examples | Why |
|---|---|---|
| Account data | Name, email, phone number, password | To create and secure your account |
| KYC data | PAN, Aadhaar (creators), GST/company documents (brands) | Identity verification, required before Payout (see KYC Policy) |
| Campaign data | Campaign briefs, Deliverables, messages, approvals | To run and record Campaigns |
| Payment data | Bank/UPI details, transaction history | To route escrow funding and Payouts through our licensed Payment Aggregator |
| Usage data | Device, IP address, log data, cookies | Security, analytics, product improvement |
| Communications | Support tickets, grievance submissions | To respond to you |

We do not store full card numbers or bank credentials ourselves — these are handled by our licensed Payment Aggregator.

## 3. Why we collect it (our legal basis)

Under the DPDP Act, we process your personal data based on:

- **Your consent** — given when you sign up, given clearly and specifically, and withdrawable at any time (withdrawal doesn't affect processing already done).
- **Legitimate uses permitted by law** — such as verifying identity for KYC/AML compliance (PMLA, RBI KYC Master Directions), deducting TDS under Section 194-O, and responding to your own requests.

We will always tell you what we're collecting and why at the point of collection, not just in this document.

## 4. How we use your data

- Operate your account and Campaigns
- Verify identity (KYC) before releasing a Payout
- Calculate and disclose Platform fees and TDS
- Detect fraud, fake engagement, or policy violations
- Improve our product, including training and evaluating AI features (Meera)
- Communicate with you about your account, Campaigns, and grievances
- Comply with Indian law (tax, AML, consumer protection)

## 5. Who we share it with

We share data only where necessary:

- **Payment Aggregator** — a licensed, RBI-authorized partner that processes escrow funding and Payouts. They receive only what's needed to process the transaction.
- **Verification providers** — for KYC document checks.
- **Cloud and AI infrastructure providers** — for hosting and AI-assisted features, under contractual confidentiality obligations.
- **Regulators and authorities** — where required by Indian law (tax, AML, law enforcement requests).

We do not sell your personal data.

## 6. Your rights under the DPDP Act

As a **Data Principal**, you have the right to:

1. **Access** — a summary of the personal data we hold about you and how we process it.
2. **Correction and updating** — ask us to correct inaccurate or incomplete data.
3. **Erasure** — ask us to delete your data, subject to our legal retention duties (see below).
4. **Withdraw consent** — at any time, for processing based on consent.
5. **Grievance redressal** — raise a complaint with our Grievance Officer, and escalate to the Data Protection Board of India if unresolved.
6. **Nominate** — name someone to exercise your rights in case of death or incapacity.

To exercise these rights, contact `info@influora.in` or our Grievance Officer (details below).

## 7. Data retention

We keep your data only as long as needed for the purpose it was collected, or as required by Indian law — for example, KYC and transaction records are retained for the periods required under PMLA and tax law. Once that period ends, we delete or anonymize your data. Specific retention periods per data category: `[RETENTION SCHEDULE — TBD, pending legal confirmation]`.

## 8. Security

We use industry-standard safeguards, including encryption of sensitive data at rest, to protect your personal data from unauthorized access, loss, or misuse. No system is 100% secure, and we continuously review our security practices.

## 9. Children's data

Influora is not intended for anyone under 18. We do not knowingly collect personal data from minors.

## 10. Cookies

We use cookies for login sessions, security, and analytics. See our Cookie Policy (`[LINK — pending Wave 2 publish]`) for details and your consent choices.

## 11. Grievances about your data

If you have concerns about how we handle your personal data, contact our Grievance Officer:

`[GRIEVANCE OFFICER — NAME / EMAIL / ADDRESS — TBD]`

We aim to acknowledge grievances within 48 hours and resolve them within 30 days. If unresolved, you may approach the Data Protection Board of India.

## 12. Changes to this policy

We may update this Privacy Policy as our practices or Indian law change. We'll post updates here with a new "Last updated" date.

## 13. Contact us

Registered office: `[REGISTERED ADDRESS — TBD]`
Support: `info@influora.in`
Grievance Officer: `[GRIEVANCE OFFICER — NAME / EMAIL / ADDRESS — TBD]`
