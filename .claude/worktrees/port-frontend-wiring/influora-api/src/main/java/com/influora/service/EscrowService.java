...
public class EscrowService {
    ...
    public EscrowStatusResponse release(AuthPrincipal principal, String workspaceId, String milestoneId) {
        ...
        // D14 Doc#2 — creator service invoice, created inside this same transaction as the release
        // posting above (gated on that posting having actually succeeded, never on "release() was
        // called"). Best-effort internally (a PDF/R2 hiccup never rolls back the release itself).
        campaignServiceInvoiceService.createAtRelease(hold, collaboration, posting.creditLeg().getId());
    ...
    public List<EscrowStatusResponse> adminSplitForDispute(
        ...
        for (EscrowHold hold : requireFrozenHoldsForCollaboration(collaborationId)) {
            ...
            if (creditLegId != null) {
                campaignServiceInvoiceService.createAtRelease(hold, collaboration, creditLegId);
            }
