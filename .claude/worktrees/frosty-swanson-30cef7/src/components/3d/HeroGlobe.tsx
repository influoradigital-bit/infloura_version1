  ...
  return (
    <group ref={groupRef}>
      <points>
        ...
        <pointsMaterial size={0.035} color={PRIMARY} transparent opacity={0.85} depthWrite={false} />
      </points>
      {/* Soft inner sphere gives the point cloud a body */}
      <mesh>
        <sphereGeometry args={[GLOBE_RADIUS * 0.98, 32, 32]} />
        <meshBasicMaterial color="#f0ebfa" transparent opacity={0.35} />
      </mesh>
      <TradeArcs />
    </group>
  );
