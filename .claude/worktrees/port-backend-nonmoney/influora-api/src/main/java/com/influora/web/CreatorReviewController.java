...
/**
 * Task #29 — creator rates brand post-{@code COMPLETED} (CEO §1.2). Identity resolved inside
 * {@link ReviewService} via {@link com.influora.service.CreatorContextService}.
 */
@RestController
@RequestMapping("/creator/reviews")
public class CreatorReviewController {

    private final ReviewService reviewService;

    public CreatorReviewController(ReviewService reviewService) {
    ...
    public ResponseEntity<ApiResponse<ReviewResponse>> create(
        ...
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(ApiResponse.ok(reviewService.createCreatorReview(principal, body)));
    }

    /** V-GA-8 — brands' reviews about the authenticated creator. */
    @GetMapping("/received")
    public ResponseEntity<ApiResponse<List<ReviewResponse>>> listReceived(
            @AuthenticationPrincipal AuthPrincipal principal) {
        return ResponseEntity.ok(ApiResponse.ok(reviewService.listReceivedByCreator(principal)));
    }
    ...
    public ResponseEntity<ApiResponse<FlagReviewResponse>> flag(
        ...
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(ApiResponse.ok(reviewService.flagCreatorReview(principal, reviewId, body)));
    }
}
