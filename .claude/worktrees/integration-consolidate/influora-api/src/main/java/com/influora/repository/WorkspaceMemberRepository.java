...
public interface WorkspaceMemberRepository extends JpaRepository<WorkspaceMember, String> {
    ...
    /**
     * H-16 — all active memberships for a user, oldest first; powers {@code
     * WorkspaceService#listMyWorkspaces}'s workspace-switch list (the list-returning counterpart
     * to {@link #findFirstByUserIdAndActiveTrueOrderByCreatedAtAsc} above, which only needs the
     * single oldest row for login).
     */
    List<WorkspaceMember> findByUserIdAndActiveTrueOrderByCreatedAtAsc(String userId);
