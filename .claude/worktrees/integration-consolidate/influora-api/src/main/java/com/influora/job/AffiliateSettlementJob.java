 ...
 * {@code @Transactional protected doSettleCreator(...)}, called from inside the {@link
 * IdempotencyService#executeOnce} supplier lambda passed by {@link #settleOneCreator}. Because
 * that lambda captured this class's own {@code this} and called {@code doSettleCreator} directly,
 * the call bypassed Spring's transactional proxy (Spring AOP's self-invocation limitation) and
 * {@code @Transactional} was a silent no-op — a failure partway through marking one creator's
 * earnings SETTLED would not roll back the earnings already saved in that same loop. The write now
 * lives on {@link AffiliateSettlementWriter}, a genuinely separate {@code @Component}, so it is
 * invoked through a real Spring proxy and {@code @Transactional} actually applies.
 */
@Component
public class AffiliateSettlementJob {
    ...
    private final AffiliateSettlementWriter affiliateSettlementWriter;
    ...
    public AffiliateSettlementJob(
        ...
        this.affiliateSettlementWriter = affiliateSettlementWriter;
    }
    ...
    @SchedulerLock(name = "AffiliateSettlementJob", lockAtMostFor = "PT2H", lockAtLeastFor = "PT1M")
    ...
    private boolean settleOneCreator(String creatorId, String periodYearMonth, AffiliateSettlementBatch batch) {
        ...
        try {
            idempotencyService.executeOnce(
                    ...
                    null,
                    IDEMPOTENCY_SCOPE,
                    // [W1-7] Through affiliateSettlementWriter (a real, separate @Component proxy)
                    // -- NOT this.doSettleCreator(...) -- so @Transactional actually applies.
                    (Supplier<Void>)
                            () -> {
                                affiliateSettlementWriter.doSettleCreator(settleable, batch);
                                return null;
                            });
