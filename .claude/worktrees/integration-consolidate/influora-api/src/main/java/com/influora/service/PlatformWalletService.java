...
public class PlatformWalletService {
    ...
    /**
     * Platform revenue wallet — receives creator-side {@code PLATFORM_FEE} postings at escrow
     * release. Distinct from the clearing wallet so fee revenue is traceable per transaction.
     */
    public static final String PLATFORM_REVENUE_WALLET_OWNER_ID = "platform-revenue-wallet";
    ...
    @Transactional
    public Wallet requireRevenueWallet() {
        return walletRepository
                .findByOwnerId(PLATFORM_REVENUE_WALLET_OWNER_ID)
                .orElseGet(this::createRevenueWallet);
    }
    ...
    private Wallet createRevenueWallet() {
        Wallet wallet = Wallet.forWorkspace(Ulids.newUlid(), PLATFORM_REVENUE_WALLET_OWNER_ID);
        return walletRepository.save(wallet);
    }
}
