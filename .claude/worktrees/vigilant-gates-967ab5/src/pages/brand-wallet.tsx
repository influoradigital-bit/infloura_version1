    ...
    <TooltipProvider>
      <div className="space-y-6">
                    ...
                    <div className="rounded-lg border border-stage-negotiating-border bg-amber-50 p-3">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="space-y-1">
                          ...
                          <button
                            onClick={() => setAddAmount(mockWalletData.suggestedRecharge.toString())}
                            className="text-lg font-bold text-amber-700 hover:underline"
                          >
                            {formatCurrency(mockWalletData.suggestedRecharge)}
                          </button>
                          <p className="text-xs text-stage-negotiating-fg">
                            Current runway: {mockWalletData.runwayDays} days | Projected burn: {formatCurrency(mockWalletData.projectedBurn30Days)}/month
                          </p>
                        </div>
                      </div>
                    </div>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          ...
          <Card className="relative overflow-hidden">
            ...
            <CardContent className="relative">
              <p className="text-sm text-muted-foreground">
                Last recharge: {formatCurrency(mockWalletData.lastRechargeAmount)} on{' '}
                {formatDate(mockWalletData.lastRecharge)}
              </p>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader className="pb-2">
              ...
              <CardTitle className="text-2xl text-amber-500">
                {formatCurrency(mockWalletData.escrowLocked)}
              </CardTitle>
            </CardHeader>
            <CardContent>
              <p className="text-sm text-muted-foreground">
                {mockEscrowItems.length} active campaigns
              </p>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader className="pb-2">
              ...
              <CardTitle className="text-2xl text-blue-500">
                {formatCurrency(mockWalletData.pendingSettlement)}
              </CardTitle>
            </CardHeader>
            <CardContent>
          ...
          <Card className={cn(
            'border-2',
            mockWalletData.runwayDays < 14 ? 'border-red-300 bg-red-50/30' :
            mockWalletData.runwayDays < 30 ? 'border-amber-300 bg-amber-50/30' :
            ...
            <CardHeader className="pb-2">
              ...
              <CardTitle className={cn(
                'text-2xl',
                mockWalletData.runwayDays < 14 ? 'text-stage-disputed-fg' :
                mockWalletData.runwayDays < 30 ? 'text-stage-negotiating-fg' :
                'text-stage-approved-fg'
              )}>
                {mockWalletData.runwayDays} days
              </CardTitle>
            </CardHeader>
            <CardContent>
              <Progress 
                value={Math.min((mockWalletData.runwayDays / 60) * 100, 100)} 
                className={cn(
                  'h-2 mb-2',
                  mockWalletData.runwayDays < 14 && '[&>div]:bg-red-500',
                  mockWalletData.runwayDays >= 14 && mockWalletData.runwayDays < 30 && '[&>div]:bg-amber-500',
                  mockWalletData.runwayDays >= 30 && '[&>div]:bg-green-500'
                )}
              />
              <p className="text-xs text-muted-foreground">
                Burn rate: {formatCurrency(mockWalletData.projectedBurn30Days)}/mo
              </p>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <Card>
            <CardHeader className="pb-2">
              ...
              <CardTitle className="text-xl flex items-center gap-2">
                {formatCurrency(mockWalletData.totalTDSDeducted)}
                <Tooltip>
          ...
          <Card>
            <CardHeader className="pb-2">
              ...
              <CardTitle className="text-xl flex items-center gap-2">
                {formatCurrency(mockWalletData.totalGSTPaid)}
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button variant="ghost" size="icon" className="h-6 w-6">
                      <Download className="h-4 w-4" />
