...
// ---------------------------------------------------------------------------
// Brand-facing creator analytics (influora-api AnalyticsController /
// AnalyticsDtos.java). Shapes mirror the backend records field-for-field so
// the hooks (useCreatorMetrics.ts / useCreatorScores.ts) map with no changes:
//   - CreatorMetrics   ≡ AnalyticsDtos.CreatorMetricsResponse
//   - MetricDataPoint  ≡ AnalyticsDtos.MetricDataPoint
//   - CreatorScores    ≡ AnalyticsDtos.CreatorScoresResponse
// Honesty rule: score fields the backend can't yet compute come back null
// (BrandSafetyScoreService isn't built) — never a fabricated value — so those
// fields are typed `| null` and consumers render a "not yet available" state.
// ---------------------------------------------------------------------------

/** A UI-selected date window for analytics queries (converted to ISO before the request). */
export interface AnalyticsDateRange {
  start?: Date;
  end?: Date;
}

/** One point in a metrics trend series (AnalyticsDtos.MetricDataPoint). */
...
/**
 * Brand-facing creator metrics (AnalyticsDtos.CreatorMetricsResponse).
 * `trendData` is populated only when a start/end range was supplied; otherwise
 * it comes back as an empty array (never fabricated).
 */
...
/**
 * Brand-facing creator scores (AnalyticsDtos.CreatorScoresResponse). Every
 * score is `| null` because the backend returns null for anything it can't
 * honestly compute yet (brandSafetyScore/garmFlags/contentSentiment are always
 * null today — BrandSafetyScoreService isn't built).
 */
export interface CreatorScores {
  authenticityScore: number | null;
  fakeFollowerReasons: string[] | null;
  ...
  computedAt: string | null; // ISO instant
}
