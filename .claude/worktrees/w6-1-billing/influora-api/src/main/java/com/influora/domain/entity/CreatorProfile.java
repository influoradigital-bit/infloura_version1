...
public class CreatorProfile {
    ...
    /**
     * N1 (Wave 6) creator onboarding identity KYC — V20260715190000__creator_identity_kyc.sql.
     * Distinct from {@link #taxRegistrationStatus} (GST business registration, D14): this is the
     * personal-identity gate ({@code onboarding/creator/kyc}, deferred to first withdrawal per the
     * onboarding UI). {@code pan} is intentionally shared with the D14 tax-identity column — same
     * real-world PAN number, one field, not two.
     */
    @Enumerated(EnumType.STRING)
    @Column(name = "identity_kyc_status", length = 20)
    private VerificationStatus identityKycStatus;

    /** Last 4 digits only — full Aadhaar number is never captured or stored (Kabir PII discipline). */
    @Column(name = "aadhaar_last4", length = 4)
    private String aadhaarLast4;

    /** R2 object key (or presigned/public URL) for the KYC selfie, from {@code POST /uploads}. */
    ...
    public static CreatorProfile newForUser(String id, String userId, String displayName) {
        ...
        p.identityKycStatus = VerificationStatus.UNVERIFIED;
    ...
    public VerificationStatus getIdentityKycStatus() {
        return identityKycStatus;
    }
    ...
    /**
     * N1 (Wave 6) — {@code onboarding/creator/kyc}. Null-guarded partial update, same discipline as
     * {@link #applyTaxIdentity}. Always moves status to {@code PENDING} on submission — verification
     * (moving to VERIFIED/REJECTED) is an admin/automated review step outside this method's scope.
     */
    public void applyIdentityKyc(String pan, String aadhaarLast4, String selfieUrl) {
        if (pan != null) this.pan = pan;
        if (aadhaarLast4 != null) this.aadhaarLast4 = aadhaarLast4;
        if (selfieUrl != null) this.selfieUrl = selfieUrl;
        this.identityKycStatus = VerificationStatus.PENDING;
        touch();
    }
    ...
    /**
     * H-10 (INFLUORA-PRODUCTION-READINESS-AUDIT-2026-07-14.md) — denormalized discovery-ranking
     * summary written by {@code PlatformStatsAggregationJob} after it rolls up {@code
